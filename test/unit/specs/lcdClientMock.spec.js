describe("LCD Client Mock", () => {
  ...
  it("builds a tx", async () => {
    let { address: fromAddr } = await client.generateKey({
      name: "foo",
      password: "1234567890"
    })
    let { address: toAddr } = await client.generateKey({
      name: "bar",
      password: "1234567890"
    })
  ...
  it("query and update the nonce", async () => {
    let nonce = await client.queryNonce(
  ...
  it("sends coins", async () => {
    ...
    let { data: account } = await client.queryAccount(
      "DF096FDE8D380FA5B2AD20DB2962C82DDEA1ED9B"
    )
