...
class StoreUpdater {
  constructor(network, apiClass, store) {
  ...
  async pollForProposals() {
    ...
    const api = new this.apiClass(this.network, this.store)
  ...
  async pollForNewBlock() {
    const api = new this.apiClass(this.network, this.store)
    await this.checkForNewBlock(api)
  ...
  async checkForNewBlock(api) {
    let block
    try {
      block = await api.getBlockByHeightV2()
    } catch (error) {
    ...
    if (block && this.height !== block.height) {
      // apparently the cosmos db takes a while to serve the content after a block has been updated
      // if we don't do this, we run into errors as the data is not yet available
      setTimeout(() => this.newBlockHandler(block, api), COSMOS_DB_DELAY)
  ...
  async newBlockHandler(block, api) {
    try {
      ...
      if (api.newBlockHandler) {
        await api.newBlockHandler(block, this.store)
      }
      ...
      this.updateValidators(block, api)
      this.handleTransactions(block)
    } catch (error) {
  ...
  updateValidators(block, api) {
      const validators = await api.getAllValidators(block.height)
      await this.store.update({
