...
export default {
  ...
  methods: {
    getVotesDeposits() {
      let proposalId = Number(this.proposal.proposal_id)
      if (this.proposal.proposal_status !== `DepositPeriod`) {
        this.$store.dispatch(`getProposalVotes`, proposalId)
      }
      this.$store.dispatch(`getProposalDeposits`, proposalId)
    },
    vote(choice) {
