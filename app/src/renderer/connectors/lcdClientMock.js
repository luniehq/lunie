...
module.exports = {
      ...
      data: {
        ...
        signature: {
          Sig: {
            type: "ed25519",
            data:
              "4631B03666EDF3D95E67A0700AD93741728E9466E51A8684F8637B935BB9A296ACE66256F2EDEC4F469FB9A940A4C1D30BB9170B5CA5292A6570A9F836DB7609"
          },
          Pubkey: {
            type: "ed25519",
            data:
              "81B11E717789600CC192B26F452A983DF13B985EE75ABD9DD9E68D7BA007A958"
          }
        }
      }
    }
  ...
  async updateKey(account, { name, password, new_passphrase }) {
  ...
  async recoverKey({ name, password, seed_phrase }) {
    // eslint-disable-line camelcase
    let key = {
      name,
      password,
  ...
  async buildDelegate({ sequence, from, amount, pub_key }) {
    // eslint-disable-line camelcase
    return {
      type: "sigs/one",
      data: {
...
function sendCoins({ inputs, outputs }) {
  if (
    state.accounts[inputs[0].address.addr].coins.find(c => c.denom === "mycoin")
      .amount < inputs[0].coins[0].amount
  ) {
    return txResult(1, "Not enought coins in your account")
  }

  // update balances
  state.accounts[inputs[0].address.addr].coins[0].amount -=
    inputs[0].coins[0].amount
