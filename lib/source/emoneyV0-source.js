...
class EMoneyV0API extends TerraV3API {
  ...
  async calculateFiatValue(balance, selectedFiatCurrency = `EUR`) {
    ...
    // e-Money acts like a trading platform where the value for the different backed tokens will be changing slightly.
    // They are providing us with an API for us to query the exchange rate from their token to the value of that token
    // For aggregating all the values of the different tokens we normalize to one currency using the public API https://exchangeratesapi.io/
    const lunieCoin = this.reducers.coinReducer(balance)
  ...
  async getBalancesFromAddress(address, fiatCurrency) {
    return TerraV3API.prototype.getBalancesFromAddress.call(
      this,
      address,
      fiatCurrency
    )
