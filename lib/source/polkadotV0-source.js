...
class polkadotAPI {
  ...
  async getAllValidators() {
    ...
    allValidators.forEach((validator) => {
      if (expectedReturns[validator.accountId.toString()]) {
        ...
        validator.expectedReturns = undefined
      }
  ...
  async getAddressRole(address) {
    if (this.store.data.bonds) {
      if (this.store.data.bonds[address] === address) {
        return `stash/controller`
      } else if (
        this.store.data.bonds[address] &&
        this.store.data.bonds[address] !== address
      ) {
        return `stash`
      } else {
        if (Object.values(this.store.data.bonds).includes(address)) {
          return `controller`
        } else {
          return `none`
        }
      ...
      throw new Error(`Bonds dictionary is not yet loaded!`)
    }
  }
  ...
  async getAllBonds() {
    const api = await this.getAPI()
    let allBonds = {}
    const allBondedEntries = await api.query.staking.bonded.entries()
    allBondedEntries.forEach(([key, bond]) => {
      allBonds[key.toHuman()] = bond.toString()
    })
    return allBonds
  }
