...
class polkadotAPI {
  ...
  async getEraRewards(era) {
    ...
    console.log('getEraRewards for number of delegators:', delegators)
  ...
  async getInactiveDelegationsForDelegatorAddress(
    delegatorAddress,
    validatorsDictionary
  ) {
    const api = await this.getAPI()
    ...
    const allDelegations = stakingInfo.raw.targets
    allDelegations.forEach(nomination => {
      inactiveDelegations.push(
        this.reducers.delegationReducer(
          this.network,
          nomination,
          validatorsDictionary[nomination.toHuman()],
  ...
  async getDelegationForValidator(delegatorAddress, validator) {
    ...
    if (!delegation) {
      ...
      const stakingInfo = await api.query.staking.nominators(delegatorAddress)
      const allDelegations = stakingInfo.raw.targets
      let inactiveDelegation = allDelegations.find(
        nomination => validator.operatorAddress === nomination.toHuman()
      )
      inactiveDelegation = this.reducers.delegationReducer(
        this.network,
        inactiveDelegation,
        validator,
        delegationEnum.INACTIVE
