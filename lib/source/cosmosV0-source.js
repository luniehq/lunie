...
class CosmosV0API extends RESTDataSource {
  ...
  // needed for reducers
  async getFiatValues(coins, fiatCurrency = 'EUR') {
    return await this.fiatValuesAPI.calculateFiatValues(coins, fiatCurrency)
  }
  ...
  async getBalancesFromAddress(address, fiatCurrency) {
    ...
    const fiatValues = await this.getFiatValues(coins, fiatCurrency)
