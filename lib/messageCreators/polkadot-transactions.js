...
async function multiMessage(transactions) {
  const api = await getAPI()
  return api.tx.utility.batch(transactions)
}

async function getSignMessage(senderAddress, transaction) {
...
async function SendTx(senderAddress, { to, amount }, network) {
  const api = await getAPI()
...
// Staking
async function StakeTx(senderAddress, { to, amount, addressRole }, network) {
...
async function RestakeTx(senderAddress, { to, from, addressRole }) {
  // stake with all existing plus the selected
  const api = await getAPI()
  const transactions = []
...
async function ClaimRewardsTx(senderAddress) {
  let allClaimingTxs = []
  const api = await getAPI()
