...
class PolkadotNodeSubscription {
  ...
  async newBlockHandler(blockHeight) {
    ...
    try {
      ...
      // needs to be done after the validators are updates as the validators are needed to resolve the rewards
      if (
        isSessionChange &&
        !(await this.eraRewardsExist(this.currentEra - 1))
      ) {
        // it should be one era less since those are the latest rewards
        this.updateRewards(
          this.currentEra,
          this.sessionValidators[0].chainId
        ).catch(error => {
  ...
  async updateRewards(era, chainId) {
    ...
    console.log(
      'store rewards',
      rewards.filter(({ amount }) => amount > 0).length
    )
    this.storeRewards(
      rewards
  ...
  deleteOldRewards(chainId, currentEra) {
    const schema_prefix = this.network.id.replace(/-/, '_')
    const table = 'rewards'
    const deleteOldRewardsQuery = `
        mutation {delete_${schema_prefix}_${table}(where:{height:{_lt: "${currentEra -
      80}"} chain_id:{ _eq: "${chainId}" }}) {
            affected_rows
          }
        }
      `
    try {
      this.db.query(deleteOldRewardsQuery)
    } catch (error) {
      console.error('Failed to delete old rewards rows', error)
      Sentry.captureException(error)
    }
  }

  storeRewards(rewards, chainId, era) {
    this.deleteOldRewards(chainId, era)
    const chunkedRewards = _.chunk(rewards, 50)
    return Promise.all(
      chunkedRewards.map(chunkedReward =>
        this.db.insert('rewards', chunkedReward, undefined, chainId)
      )
    ) // height is in the rewards rows already
  }
