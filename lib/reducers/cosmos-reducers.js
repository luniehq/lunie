...
function getValidatorStatus(validator) {
  if (validator.status === 2) {
    return {
      status: 'active',
      status_detailed: 'Validator is actively validating'
    }
  }
  if (
    ...
    return {
      status: 'inactive',
      status_detailed: 'Validator is banned from the network'
    }
  }

  return {
    status: 'inactive',
    status_detailed: 'Validator is currently not validating'
  }
}
