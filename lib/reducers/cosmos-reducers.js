...
function proposalBeginTime(proposal) {
  switch (proposal.proposal_status.toLowerCase()) {
    case "deposit_period": return proposal.submit_time
    case "voting_period": return proposal.voting_start_time
    case "passed":
    case "rejected": return proposal.voting_end_time
  }
}

function proposalEndTime(proposal) {
  switch (proposal.proposal_status.toLowerCase()) {
    case "deposit_period": return proposal.voting_start_time
    case "voting_period":
...
function getDeposit(proposal, bondDenom) {
  return atoms(proposal.total_deposit.reduce((sum, cur) => sum.plus(cur.denom === bondDenom ? cur.amount : 0), BigNumber(0)))
}

function proposalReducer(proposal, bondDenom) {
  return {
    ...
    tally: {
      ...
      total: atoms(BigNumber(proposal.final_tally_result.yes).plus(proposal.final_tally_result.no).plus(proposal.final_tally_result.abstain).plus(proposal.final_tally_result.no_with_veto))
    },
    deposit: getDeposit(proposal, bondDenom),
  }
}
