...
function fixDecimalsAndRoundUp(number, decimalsNumber) {
  return (number.toFixed(decimalsNumber) * decimalsNumber) / decimalsNumber
}
...
function rewardCoinReducer(reward) {
  ...
  const mappedMultiDenomRewardsArray = multiDenomRewardsArray.map(
    reward =>
      (reward = {
        denom: denomLookup(reward.match(/[a-z]+/gi)),
...
async function balanceReducer(
  ...
  return {
    ...coin,
    fiatValue: calculateFiatValue
      ? await calculateFiatValue(coin, fiatCurrency)
      : undefined,
    gasPrice: gasPrices
      ? gasPriceReducer(
...
async function balanceV2Reducer(
  ...
  return {
    ...
    availableFiatValue: calculateFiatValue
