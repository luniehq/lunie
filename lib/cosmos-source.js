...
class CosmosAPI extends RESTDataSource {
  ...
  async getAllValidators(delegatorAddress) {
    let [
      validators,
      annualProvision,
      validatorSet,
    ...
    // filter the validators to return by the ones that the delegator has a delegation to
    if (delegatorAddress) {
      const delegationsDictionary = _.keyBy(delegations, 'validatorAddress')
      validators = validators.filter(
        ({ operator_address }) => !!delegationsDictionary[operator_address]
      )
    }
