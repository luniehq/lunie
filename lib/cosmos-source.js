...
class CosmosAPI extends RESTDataSource {
  ...
  async subscribeToBlocks(network) {
    ...
    this.wsClient.subscribe({ query: "tm.event='NewBlock'" }, async event => {
      ...
      const txs = await await this.get(
        `txs?tx.height=${event.block.header.height}`
      )
      txs.forEach(tx => {
        tx.tags.forEach(tag => {
          if (tag.value.startsWith(`cosmos`)) {
            const fmtTx = transactionReducer(tx)
            chainpubsub.publishUserTransactionAdded(
              network.id,
              tag.value,
              fmtTx
            )
  ...
  async getTransactionsByHeight(height) {
    ...
    return Array.isArray(response)
      ? response.map(transaction => transactionReducer(transaction))
      : []
  }
  ...
  async getTransactions(address) {
    const pagination = `&page=${1000000000}`
