...
async function validator(_, { networkId, address }, { dataSources }) {
  const validator = await selectFrom(
    dataSources,
    networkId
  ).getValidatorByAddress({ address })
  const response = (await dataSources.LunieDBAPI.getValidatorInfoByAddress(
    address,
    networkId
  )).shift()
  if (
    validator &&
    response &&
    validator.operatorAddress == response.operator_address
  ) {
