...
const networkFees = async (_, { networkId, transactionType }) => {
  return {
    gasEstimate: getNetworkTransactionGasEstimates(networkId, transactionType),
    gasPrices: getNetworkGasPrices(networkId),
    chainAppliedFees: getNetworkTransactionChainAppliedFees(
      networkId,
      transactionType
    )
  }
...
const resolvers = {
  ...
  Query: {
    ...
    accountRole: async (_, { networkId, address }, { dataSources }) => {
      await localStore(dataSources, networkId).dataReady
      if (!remoteFetch(dataSources, networkId).getAddressRole)
        throw new Error('Not a Polkadot network!')

      return remoteFetch(dataSources, networkId).getAddressRole(address)
    }
  },
  Subscription: {
    blockAdded: {
    ...
    notificationAdded: {
      subscribe: (_, { addressObjects }) => notificationAdded(addressObjects)
    },
    userTransactionAdded: {
