...
function selectFrom(dataSources, networkId) {
  switch (networkId) {
    ...
    case 'terra-testnet':
      return dataSources.TerraTestnetAPI
    case 'livepeer-mainnet':
      return dataSources.LivepeerMainnetAPI
    default:
...
async function enrichValidator(validatorInfo, validator) {
...
async function validators(
  ...
  // TODO these updates should happen already in the store
  ...
  const applyEnrichValidators = async validators => {
    return Promise.all(
  ...
  validators = await applyEnrichValidators(validators)
...
const resolvers = {
  ...
  Query: {
    ...
    networks: (_, __, { dataSources }) => {
      const networks = Object.values(dataSources.store)
        .filter(({ network }) => network.id !== 'livepeer-mainnet')
        .map(({ network }) => {
          if (network.id === 'local-cosmos-hub-testnet') {
            // HACK: network.api_url for the testnet has to be different for internal
            // (docker DNS to access the testnet container) and external (this frontend to
