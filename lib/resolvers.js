...
async function validators(_, { networkId, addressList }, { dataSources }) {
  ...
  return dataSources.LunieDBAPI.extendValidators(validators, networkId);
}
...
const resolvers = {
  Query: {
    ...
    validators,
    validator: async ( _, { networkId, address }, { dataSources }) => dataSources.LunieDBAPI.extendValidator(await selectFrom(dataSources, networkId).getValidatorByAddress({ address }), networkId),
