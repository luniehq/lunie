...
async function enrichValidator(dataSources, networkId, validator) {
  ...
  // TODO refactor
  if (validator && !validatorInfo && !validator.name) {
    return {
      ...validator,
...
async function validators(
  ...
  validators = validators.map(validator => {
    ...
    if (validatorProfile && validatorProfile.picture) {
      validator.name = validatorProfile.name
      validator.picture = validatorProfile.picture
    } else if (validatorProfile && validatorProfile.name) {
      validator.name = validatorProfile.name
      validator.picture = ''
    } else {
      if (!validator.name) {
        // hack to keep the FE agnostic
