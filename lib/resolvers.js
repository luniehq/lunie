...
const networkFees = async (
  _,
  { networkId, senderAddress, messageType, transactionType, message, memo }
) => {
  const networkObject = networks.find(({ id }) => id === networkId)
  return {
    gasEstimate: getNetworkTransactionGasEstimates(networkId, messageType),
    gasPrices: getNetworkGasPrices(networkId),
    chainAppliedFees: getNetworkTransactionChainAppliedFees(
      networkId,
      messageType || transactionType // DEPRECATE transactionType
    ),
    fee:
      networkObject.network_type === 'polkadot'
        ? await getPolkadotFee({
