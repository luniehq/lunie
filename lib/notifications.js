...
function getMessageTitle(notification) {
  ...
  switch (notification.eventType) {
    ...
    case eventTypes.TRANSACTION_SEND:
      return `Transaction sent with ID ${
        data.hash
      } for network ${findNetworkTitle(notification.networkId)}`
...
function getIcon({ eventType }) {
  switch (eventType) {
    ...
    case eventTypes.TRANSACTION_SEND:
      return `/img/icons/activity/Send.svg`
...
const startNotificationService = () => {
  const dbInstance = database(config)('')
...
const getNotifications = async (
  ...
  const dbInstance = database(config)('')
  const relevantNotifications = await dbInstance.getNotifications(
