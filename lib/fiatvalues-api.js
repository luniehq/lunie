...
class fiatValueAPI {
  constructor() {
    ...
    this.coins = ['ATOM', 'LUNA', 'KRT', 'SDT', 'KAVA'] // Lunie coins currently being traded in the open
    this.priceFeed = []
      ...
      coins.map(coin =>
        ...
        this.allFiatValues[coin.denom][fiatCurrency]
          ? {
              ...
              amount: this.fixDecimalsAndRoundUp(
                Number(coin.amount) *
                  this.allFiatValues[coin.denom][fiatCurrency],
                6
              ),
  ...
  fixDecimalsAndRoundUp(number, decimalsNumber) {
    return (
