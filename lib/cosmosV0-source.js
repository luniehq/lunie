...
class CosmosV0API extends PerBlockCacheDataSource {
  ...
  async loadStaticData() {
    // we wait for the remote API to be up before querying static data
    // if not those request fail and will never be loaded again
    // happens in e2e test setups, where booting of testnet and API happens very close
    await this.awaitUp()
