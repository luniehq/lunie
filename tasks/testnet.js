...
async function main() {
  ...
  const genesisJson = await get(
    `https://github.com/tendermint/testnets/raw/master/${network}/gaia/genesis.json`
  ).catch(e => {
    throw new Error(`Can't load genesis.json: ${e.message}`)
  })
  const configToml = await get(
    `https://github.com/tendermint/testnets/raw/master/${network}/gaia/config.toml`
  ).catch(e => {
    throw new Error(`Can't load config.toml: ${e.message}`)
  })
  const gaiaVersionTxt = await get(
    `https://github.com/tendermint/testnets/raw/master/${network}/gaia/gaiaversion.txt`
  ).catch(e => {
    throw new Error(`Can't load config.toml: ${e.message}`)
  })
  const path = join(
