    ...
    <div v-focus-last class="action-modal" tabindex="0" @keyup.esc="close">
      ...
      <template v-else>
        ...
        <div v-else-if="step === feeStep" class="action-modal-form">
          ...
          <TmFormGroup
            v-if="transactionType === messageType.CLAIM_REWARDS && this.network.id.startsWith('cosmos-hub')"
...
export default {
  ...
  computed: {
    ...
    donation() {
      if (
        !this.network.id.startsWith('cosmos-hub') ||
        this.transactionType !== messageType.CLAIM_REWARDS
      ) {
        return undefined
      }
      // for now we just pick the staking denom as donation
      const amount =
        this.rewards
          .filter(({ denom }) => denom === this.network.stakingDenom)
          .reduce((total, { amount }) => total + Number(amount), 0) || 0
      const donation = Number(amount) * this.donationPercentage
      return donation
    },
    donationOnChain() {
      return {
        ...
        denom: toMicroDenom(this.network.stakingDenom)
      }
    },
    isValidChildForm() {
